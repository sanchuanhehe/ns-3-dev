...
class VhtConfiguration : public Object
{
  public:
    ...
    using SecondaryCcaSensitivityThresholds =
        std::tuple<units::power::dBm_t, units::power::dBm_t, units::power::dBm_t>; //!< Tuple identifying CCA sensitivity thresholds for
                                         //!< secondary channels
     ...
     */
    const std::map<MHz_u, units::power::dBm_t>& GetSecondaryCcaSensitivityThresholdsPerBw() const;

    bool m_160MHzSupported; ///< whether 160 MHz operation is supported

  private:
    std::map<MHz_u, units::power::dBm_t>
