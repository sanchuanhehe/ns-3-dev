...
class TcpAbeToggleTest : public TestCase
{
  ...
  public:
    ...
    void DoRun() override
    {
        ...
        NS_TEST_EXPECT_MSG_EQ(m_socket->GetTcb()->m_useAbe, true, "ABE should be enabled");
        NS_TEST_EXPECT_MSG_EQ(m_socket->GetTcb()->m_useEcn,
                              true,
                              "ECN should be enabled along with ABE");

        m_socket->SetUseAbe(false);
        NS_TEST_EXPECT_MSG_EQ(m_socket->GetTcb()->m_useAbe, false, "ABE should be disabled");
    }
};
 ...
 *
 * This test verifies the correct behavior of congestion control algorithms
 * when ABE is enabled or disabled.
 * ABE should only work for TcpCubic, TcpNewReno, TcpLinuxReno
 */
...
class TcpAbeTestSuite : public TestSuite
{
  public:
        ...
        : TestSuite("tcp-abe-test", Type::UNIT)
    {
        ...
        AddTestCase(
            new TcpAbeTest(0, 1, 1000, 700, 100, cong_control_type, "Test TCP CUBIC without ABE"),
            TestCase::Duration::QUICK);
        AddTestCase(
            new TcpAbeTest(1, 1, 1000, 850, 100, cong_control_type, "Test TCP CUBIC with ABE"),
            TestCase::Duration::QUICK);

        cong_control_type = TcpLinuxReno::GetTypeId();
        AddTestCase(new TcpAbeTest(0,
                                   1,
                                   1000,
                                   500,
                                   100,
                    ...
                    TestCase::Duration::QUICK);
        AddTestCase(
            new TcpAbeTest(1, 1, 1000, 700, 100, cong_control_type, "Test TCP Linux Reno with ABE"),
            TestCase::Duration::QUICK);
        ...
        AddTestCase(new TcpAbeTest(0,
                    ...
                    TestCase::Duration::QUICK);
        AddTestCase(
            new TcpAbeTest(1, 1, 1000, 700, 1000, cong_control_type, "Test TCP New Reno with ABE"),
            TestCase::Duration::QUICK);
    }
};
