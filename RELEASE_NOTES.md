...
- (propagation) Added a new `HexagonalWraparoundModel` allowing users from border sites to communicate with opposite sites, given a hexagonal deployment of 0, 1 or 3 rings. `HexagonalWraparoundModel` is supported by ns-3 UMa, RMa, and UMi 3GPP propagation models that are implemented in `ThreeGppUmaPropagationLossModel`,`ThreeGppRmaPropagationLossModel`, and `ThreeGppUmiStreetCanyonPropagationLossModel`, respectively.
