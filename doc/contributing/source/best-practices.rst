...
As presented by Mikhail Matrosov's "Refactor or die" talk in CppCon 2017,
mixing refactoring and functional changes is a bad practice.

It is considered a bad practice because refactoring may involve a lot
of different files, classes, API changes to accommodate a new parameter,
for example. If a functional change is mixed in between, there is a higher
probability of introducing a bug. This results in more code to inspect and debug
to isolate and fix the bug. Which translates to unproductive work, extra hours,
working on weekends, etc, etc.

The figure below depicts the scope differences in refactoring and functional changes.

.. image:: figures/cppcon2017-refactorOrDie-scope.png
...
.. image:: figures/cppcon2017-refactorOrDie-probabilityOfBug.png

The complete talk is available in https://youtu.be/fzmjXK9JZ9o.
...
This can be achieved through commit history rewriting.
Which can also introduce new bugs, if commits are not properly rewritten.
...
This can be achieved automatically with the use of the `./ns3 --compile-or-die base_commit head_commit`.
...
compilation or causing one of the existing tests to fail, which serves as anecdotal evidence that
